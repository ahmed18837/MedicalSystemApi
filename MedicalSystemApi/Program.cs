...
{
    options.Password.RequireDigit = true; //  يتطلب رقم
    options.Password.RequireLowercase = false; // ? لا يتطلب حرفًا صغيرًا
    options.Password.RequireUppercase = false; // ? لا يتطلب حرفًا كبيرًا
    options.Password.RequireNonAlphanumeric = false; // ? لا يتطلب رمزًا خاصًا (@, #, !)
    options.Password.RequiredLength = 8; // الحد الأدنى لطول كلمة المرور (يمكنك تغييره)
})
...
app.UseMiddleware<ExceptionMiddleware>(); // تفعيل ExceptionMiddleware
